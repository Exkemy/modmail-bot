...
function getInboxGuild() {
  ...
  if (! inboxGuild) throw new BotError("Le bot n'est pas sur le serveur de boîte de réception !");
  return inboxGuild;
}
...
function getMainGuilds() {
  ...
  if (mainGuilds.length !== config.mainServerId.length) {
    if (config.mainServerId.length === 1) {
      console.warn("[AVERTISSEMENT] Le bot n'a pas rejoint le serveur principal !");
    } else {
      console.warn("[AVERTISSEMENT] Le bot n'a pas rejoint un ou plusieurs serveurs principaux !");
    }
  }
...
function getLogChannel() {
  ...
  if (! _logChannel) {
    throw new BotError("Salon de journalisation (logChannelId) introuvable !");
  }

  if (! (_logChannel instanceof Eris.TextChannel)) {
    throw new BotError("Assurez-vous que l'option logChannelId pointe vers un salon textuel !");
  }
...
async function getSelfIp() {
  if (! cachedIpPromise) {
    ...
    cachedIpPromise = publicIp({ timeout: 1000 })
      .catch(err => {
        console.warn(`Erreur lors de la récupération de l'IP publique : ${err}`);
...
const humanizeDelay = (delay, opts = {}) => humanizeDuration(delay, Object.assign({conjunction: " et "}, opts));
