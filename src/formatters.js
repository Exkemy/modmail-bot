...
const defaultFormatters = {
 ...
 formatStaffReplyEditNotificationThreadMessage(threadMessage) {
    ...
    let content = `**${originalThreadMessage.user_name}** (\`${originalThreadMessage.user_id}\`) a modifié la réponse \`${originalThreadMessage.message_number}\``;
  ...
  formatStaffReplyDeletionNotificationThreadMessage(threadMessage) {
    const originalThreadMessage = threadMessage.getMetadataValue("originalThreadMessage");
    let content = `**${originalThreadMessage.user_name}** (\`${originalThreadMessage.user_id}\`) a supprimé la réponse \`${originalThreadMessage.message_number}\``;
  ...
  formatLog(thread, threadMessages, opts = {}) {
    ...
    const lines = threadMessages.map(message => {
      ...
      if (message.message_type === THREAD_MESSAGE_TYPE.FROM_USER) {
        line += ` [UTILISATEUR] [${message.user_name}] ${message.body}`;
      } else if (message.message_type === THREAD_MESSAGE_TYPE.TO_USER) {
        if (opts.verbose) {
          line += ` [VERS UTILISATEUR] [${message.message_number || "0"}] [${message.user_name}]`;
        } else {
          line += ` [VERS UTILISATEUR] [${message.user_name}]`;
        }
        ...
        if (message.use_legacy_format) {
          ...
          line += ` (Anonyme) ${message.role_name} : ${message.body}`;
          } else {
          line += ` (Anonyme) Modérateur : ${message.body}`;
          }
        } else {
          if (message.role_name) {
        ...
        line += ` [BOT VERS UTILISATEUR] ${message.body}`;
      } else if (message.message_type === THREAD_MESSAGE_TYPE.CHAT) {
        line += ` [DISCUSSION] [${message.user_name}] ${message.body}`;
      } else if (message.message_type === THREAD_MESSAGE_TYPE.COMMAND) {
        line += ` [COMMANDE] [${message.user_name}] ${message.body}`;
        ...
        line += ` [RÉPONSE MODIFIÉE] ${originalThreadMessage.user_name} a modifié la réponse ${originalThreadMessage.message_number} :`;
        line += `\n\nAvant :\n${originalThreadMessage.body}`;
        line += `\n\nAprès :\n${message.getMetadataValue("newBody")}`;
      } else if (message.message_type === THREAD_MESSAGE_TYPE.REPLY_DELETED) {
        const originalThreadMessage = message.getMetadataValue("originalThreadMessage");
        line += ` [RÉPONSE SUPPRIMÉE] ${originalThreadMessage.user_name} a supprimé la réponse ${originalThreadMessage.message_number} :`;
        line += `\n\n${originalThreadMessage.body}`;
      } else {
        line += ` [${message.user_name}] ${message.body}`;
      }
    ...
    const header = `# Fil Modmail n°${thread.thread_number} avec ${thread.user_name} (${thread.user_id}) commencé à ${openedAt}. Toutes les heures sont en UTC+0.`;
...
module.exports = {
  formatters: new Proxy(formatters, {
    set() {
      throw new Error("Veuillez utiliser les fonctions de configuration des formateurs au lieu de modifier directement les formateurs");
    },
  }),
