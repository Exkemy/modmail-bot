...
greetingMessage[] = Voici la deuxième ligne du message de bienvenue.
greetingMessage[] = 
...
**Default:** `Votre compte Discord n'est pas assez ancien pour contacter Modmail.`  
...
To send internal messages in the thread when this option is enabled, add your command prefix (e.g. `!`) and a space at the beginning of the messages. For example, `! Ceci est un message interne`.
...
#### categoryAutomation.newThreadFromReason.REASON_KEY
**Default:** *None*
Assigns a specific category for threads opened with the given ticket reason. Use the reason identifiers defined by the bot (for example `report_member`, `appeal_punishment`, `technical_issue`, or `event_participation`). Example:

```ini
# Threads opened with the "Contester une sanction" reason (identifier: appeal_punishment)
categoryAutomation.newThreadFromReason.appeal_punishment = 1430562076912259094
```

If no matching entry is found, the bot falls back to `categoryAutomation.newThreadFromServer` and then `categoryAutomation.newThread`.
...
commandAliases.replysus = reply Merci pour cela, nous allons suspendre ce fil le temps de mener nos investigations.
...
```ini
greetingMessage[] = Bienvenue sur le serveur !
greetingMessage[] = N'oubliez pas de lire le règlement.
```
...
**Default:** `off`  
If enabled, the bot attempts to ignore common "accidental" messages that would start a new thread, such as "ok", "merci", etc.
...
**Default:** `Vous n'êtes pas membre du serveur depuis assez longtemps pour contacter Modmail.`  
...
#### responseMessage
**Default:** `Merci pour votre message ! Notre équipe de modération vous répondra ici dès que possible.`  
...
```ini
serverGreetings.94882524378968064.message = Bienvenue sur le serveur ID 94882524378968064 !
serverGreetings.94882524378968064.attachment = greeting.png

serverGreetings.541484311354933258.message[] = Bienvenue sur le serveur ID 541484311354933258 !
serverGreetings.541484311354933258.message[] = Deuxième ligne du message de bienvenue.
```
...
#### status
**Default:** `Envoyez-moi un message pour obtenir de l'aide`  
...
* You can use `{duration}` in the text to include the duration (e.g. `4 semaines, 2 jours`)
* You can use `{timestamp}` in the text to create a Discord timestamp of the time the user is blocked until (e.g. `<t:{timestamp}:f>` would become `3 juin 2022 à 23 h 50`)
...
* You can use `{delay}` in the text to include the time until the user will be unblocked (e.g. `4 semaines, 2 jours`)
* You can use `{timestamp}` in the text to create a Discord timestamp of the unblock time (e.g. `&lt;t:{timestamp}:f&gt;` would become `3 juin 2022 à 23 h 50`)
...
{
  "greetingMessage": "Bienvenue sur le serveur !\
Voici la deuxième ligne du message de bienvenue."
}
```
